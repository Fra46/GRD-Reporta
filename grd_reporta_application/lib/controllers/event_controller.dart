...
class EventController extends GetxController {
  ...
  Future<void> createEvent({
    ...
    try {
      ...
      if (fotos.isNotEmpty) {
        ...
        fotosUrls = await CloudinaryService.uploadMultiple(
          ...
          onProgress: (current, total) {
            uploadProgress.value = total > 0 ? current / total : 0;
            if (current < total) {
              uploadStatus.value = 'Subiendo foto ${current + 1} de $total...';
            } else {
      ...
      final event = EventModel(
        ...
        fechaEvento: fechaEvento ?? DateTime.now(),
  ...
  Future<void> updateEvent({
    ...
    try {
      ...
      await _db.collection('events').doc(eventId).update(updateData);
      ...
      Get.snackbar(
        'Error',
        'No fue posible actualizar el evento: $e',
        snackPosition: SnackPosition.BOTTOM,
      );
    } finally {
      isLoading.value = false;
  ...
  int get totalCriticos => events.where((e) => e.criticidad == 'alta').length;
  int get totalConAfectacion => events.where((e) => e.hayAfectacion).length;
  int get totalEdan => events.where((e) => e.requiereEdan).length;
  int get totalAbiertos => events.where((e) => e.estado == 'abierto').length;
  int get totalEnProceso =>
      events.where((e) => e.estado == 'en_proceso').length;
  int get totalCerrados => events.where((e) => e.estado == 'cerrado').length;
